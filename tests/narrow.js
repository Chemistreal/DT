/* ============================================================
   ...
   **있는지**만 본다 — 태그가 있어도 안이 넘치면 화면은 옆으로 밀린다.
   ...
   처음 재어 보니 145장 가운데 한 장이 걸렸다(2026-08-10) —

       OMR_answer_keys.html   문서 399px > 화면 360px
         div.it · span.bub · span.lab · span.c

   그 장은 **종이로 뽑는 시트**라 A4 를 기준으로 짜여 있다. 한 줄에
   번호 26px + 답 다섯 개(동그라미 17px + 이름표)가 들어가야 해서 세 칸으로는
   360px 에 안 들어간다. 칸 수를 줄이면 종이에서 시트 모양이 달라지므로
   **종이는 그대로 두고** 좁은 화면에서만 시트가 제 상자 안에서 구르게 했다.
