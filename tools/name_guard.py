...
그런데 그 규칙을 지키는 것이 사람의 기억뿐이었다. 이 저장소에서 실제로
이렇게 깨져 있었다(2026-09-02 에 재어 보고 알았다):

  · roster.html 의 `DEFAULT_ROSTER` — 실제 학생 **여든한 명**의 이름이
    그대로. 이 화면은 암호도 noindex 도 없이 Pages 로 열린다.
  · apps-script.gs 의 `seedRosterDefault()` — 같은 여든한 명. 한 번 쓰고
    끝났어야 할 초기화 함수가 명단을 영영 안고 있었다.
  · pending.html 의 데모 자료와 tests/run.js 의 픽스처 — 실제 학생 이름과
    학교가 «예시» 라는 이름으로 섞여 있었다.

앞의 둘은 «기본값» 이라서 아무도 다시 안 봤고, 뒤의 둘은 «가짜 자료» 라고
불려서 아무도 의심하지 않았다. 사람의 눈으로 다시 안 걸린다.
...
# 않게 하는 것뿐이다.
SALT = 'chemistreal/DT·학생실명·v1'

# 안 보는 곳. 지난 응시 기록(backup)은 시트에서 내려받은 것이라 이름이 있고,
# 그것은 별건으로 다룬다(여기서 잡으면 매번 빨간불이라 아무도 안 읽게 된다).
# backup/ 은 서버가 **코드로 바꿔** 올리는 명단 스냅숏이라 이름이 없다.
# 그래도 안 본다 — 시트에서 받은 것이 섞이면 매번 빨간불이 되고,
# 매번 빨간불인 검사는 아무도 안 읽는다.
SKIP_DIRS = {'.git', 'backup', 'node_modules', '__pycache__', 'demos'}
