export const meta = {
  name: 'dt-lecmap-verify',
  description: '이미 배선된 오개념→강의를 강의 본문과 대조해 적대적으로 반박한다',
  phases: [
...
/* ── 왜 「반박만」 대본이 따로 있나 ────────────────────────────────────
   배선은 끝났는데 반박이 중간에 끊기는 일이 실제로 났다(세션 한도).
   그때 남는 것은 **아무도 확인하지 않은 배선**이다. 처음부터 다시 돌리면
   이미 고른 것을 버리게 되고, 그냥 표에 옮기면 확인 안 한 문이 학생에게
   열린다. 그래서 배선 단계만 떼어 낸 대본을 둔다. */
phase('반박')
...
const results = await pipeline(
  ...
  function (key) {
    const p = '이어진 배선을 **반박한다**. 통과시키는 것이 아니라 틀린 것을 찾는 것이 목적이다.\n\n'
      + '⚠ 이 배선은 **확인을 못 거친 채 남아 있는 것**이다(앞선 실행이 중간에 끊겼다).\n'
      + '  아무도 아직 이 판단을 검토하지 않았다. 네가 첫 독자다.\n\n'
      + '조각: `' + WIP + '/' + key + '.json`\n'
