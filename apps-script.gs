...
/* 오개념 대표 이름 표 — chemengine.js 의 MIS_CANON·misCanon 과 **글자까지 같다**(tools/engine_sync.py --check 가
   잰다 · 표는 chemengine.js 에서만 고치고 여기는 --write 로 맞춘다). `옥텟규칙`/`옥텟 규칙` 처럼 표기·어순만
   다른 이름을 집계(cumulative_ 의 고질)에서만 대표 이름으로 본다. 시트의 오개념 열(원본 이름)은 손대지 않는다 —
   화면(report.html)의 이름 맞대기(misSame·misKeyIn)가 같은 표로 원본 이름과 잇는다. */
...
/* ★ 편집기에서 1회 실행: 어떤 회차를 이미 통과했는데 그 통과 시도보다 뒤(더 높은 시도)의
   ...
   - 예: 정시 통과 -> 모든 재시 삭제 / 재시 통과 -> 재재시 이상 삭제 (홍길동 케이스)
...
/* ★ 편집기에서 1회 실행: 정시(첫 응시)가 테스트(TEST)로 잘못 저장됐는데 같은 회차에
   ...
   (관리자가 테스트 모드가 켜진 채로 정시를 수기 채점한 경우 복구용 · 홍길동 케이스)
...
function cumulative_(rows) {
  ...
  use.forEach(function (r) {
    (r.wrongMis || []).forEach(function (m) {
      var mk = misCanon(m);   // 집계에서만 대표 이름 — chemengine.js cumulative·spacedReview 와 같은 규칙
      (mr[mk] || (mr[mk] = {}))[r.course + '#' + r.round] = 1;
    });
  });
...
function computePending_(activeDays) {
  ...
  /* ── 「자기는 통과했다는데 왜 재시죠?」 (2026-08-15) ────────────────
     선생님이 물으셨다 — 이몽룡 학생에게 화학Ⅱ 9회 재시 안내가 나갔는데
     학생은 통과했다고 한다.
  ...
  Object.keys(groups).forEach(function (k) {
    ...
    /* 학생이 이름 칸에 '홍길동 두레중' 이라고 적고 냈을 수 있다. 창구에서
...
/* ── 명단 한 칸 ──────────────────────────────────────────────────────
   ...
   `홍길동`(한별중)과 `홍길동`(두레중) 두 학생이 들어왔다. 이름만으로는
   ...
   ⚠ 학교는 **가르는 데만** 쓴다. 학부모에게 가는 문자는 예전 그대로
     '홍길동 학생' 이다 — '홍길동 한별중 학생' 이 아니다. */
/* ── 이름에 학교가 붙어 온 것 ────────────────────────────────────────
   명단에 학생을 두 명 넣을 방법이 없던 때, 이름 칸에 `홍길동 두레중` 처럼
   학교를 같이 적어 구분해 두었다. 그 값이 그대로 문자에 실려

       [다원교육 영재관 · 화학] 홍길동 두레중 학생 시험 안내
   ...
   ⚠ 학교로 볼 꼬리는 두 자 이상이고 초/중/고 로 끝나는 말이다.
     `홍길동 두레중` · `홍길동 두레중학교` · `홍길동(두레중)` 다 걸린다. */
...
function stuName_(x) {
  ...
  return splitSchool_(raw).name;          // '홍길동 두레중' → '홍길동'
}
...
/* 명단을 여기 적어 두지 않는다 (2026-09-02)
   ─────────────────────────────────────────────────────────────────
   이 자리에는 `seedRosterDefault()` 가 있었고, 그 안에 실제 학생 여든한
   명의 이름과 학교 둘이 그대로 박혀 있었다. 이 파일은 **공개 저장소**에
   있고 GitHub 에서 누구나 읽는다. 한 번 쓰고 마는 초기화 함수 때문에
   아이들 이름이 영영 공개되어 있었던 셈이다.

   명단은 이미 시트(_roster 탭)에 들어가 있고, 이후 편집은 roster.html
   에서 한다. 그러니 저 함수는 할 일이 끝난 지 오래였다.

   새 명단으로 갈아엎어야 하면 아래를 쓴다 — 명단은 **스크립트 속성**에
   붙여 넣고, 이 파일에는 아무 이름도 남기지 않는다.

     1) 편집기 → 프로젝트 설정 → 스크립트 속성에
        ROSTER_SEED = [{"label":"화학1 일6-10","students":["…","…"]}, …]
     2) 편집기에서 seedRosterFromProperty() 한 번 실행
     3) 끝나면 스크립트 속성에서 ROSTER_SEED 를 **지운다** */
function seedRosterFromProperty() {
  var raw = PropertiesService.getScriptProperties().getProperty('ROSTER_SEED');
  if (!raw) return '스크립트 속성 ROSTER_SEED 가 비어 있습니다 (설명을 보세요)';
  var arr;
  try { arr = JSON.parse(raw); } catch (e) { return 'ROSTER_SEED 가 JSON 이 아닙니다: ' + e; }
  if (!(arr && arr.length)) return 'ROSTER_SEED 에 반이 없습니다';
  var n = setRoster_(arr).length;
  return n + '개 반 명단 초기화 완료 · 이제 스크립트 속성에서 ROSTER_SEED 를 지우세요';
}
 ...
 *  기존에 어긋난 링크(예: 새얼중학교-장보고)를 학생키에 맞게 일괄 교정합니다.
